  ...
  return (
    <div>
      ...
      <nav className='flex flex-row justify-center'>
        <button className='mt-8 mr-8 shadow-sm border-2 rounded-2xl p-3 w-36 border-indigo-600 bg-indigo-600 text-white font-bold uppercase tracking-widest hover:bg-indigo-300 hover:text-gray-900' onClick={showShort}>short</button>
        <button className='mt-8 mr-8 shadow-sm border-2 rounded-2xl p-3 w-36 border-indigo-600 bg-indigo-600 text-white font-bold uppercase tracking-widest hover:bg-indigo-300 hover:text-gray-900' onClick={showLong}>long</button>
      </nav>
      {!hidden &&
        <section className='text-left px-28 py-20'>
          <article>
            <h1 className='text-4xl text-indigo-600 mb-6 font-light'>short & sweet</h1>
            <p className='text-2xl'>hi! my name is kelli landry. thanks for droppin&apos; in! first off, i swear i know how to use capital letters; settle into the chill of lowercase 🏄🏻‍♀️</p>
            <p className='text-2xl mt-6'>i'm a former social worker turned front-end web developer. in 2018, i started my tech journey at a louisiana-based startup. from there, i've been living life at rocket speed! my ultimate career goal is to be a badass full-stack developer. when i'm not learning something on udemy, perusing stackoverflow, or trying out some new software, i enjoy reading in my hammock, camping, dancing, playing pool, karaoke, live music, learning spanish & french on duolingo, and visiting with friends.</p>
          </article>
        </section>}
      {hidden &&
        <section className='text-left px-28 py-20 text-2xl'>
          <article>
            ...
            <p>hello! my name is kelli landry! thanks for coming along for the ride. first off, i swear i know how to capitalize. but i like the chill factor associated with lowercase 😎 anyway, you&apos;re here for the long version, so come on in; it&apos;s kinda bumpy.</p>
          </article>
          <article>
